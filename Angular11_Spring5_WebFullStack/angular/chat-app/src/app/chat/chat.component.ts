...
export class ChatComponent implements OnInit {
  ...
  mensajes: Mensaje[] = [];
  private escribiendo: string;
  clienteId: string;

  constructor() {
    //this.clienteId = 'id-' + new Date().getUTCMilliseconds() + '-' + Math.random().toString(36).substr(2); //getUTCMilliseconds() es menos preciso
    this.clienteId = 'id-' + new Date().getTime() + '-' + Math.random().toString(36).substr(2);  //getTime() es mas preciso
  }
